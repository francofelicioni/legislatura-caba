...
function getBloqueStyle(b) {
  for (const [k,v] of Object.entries(BLOQUES_CFG)) { if (b && b.includes(k)) return v; }
  return { color:"#64748b", bg:"#EFF6FF", abrev:(b||'').slice(0,6) };
}
function getBloqueBucket(b) {
  for (const k of Object.keys(BLOQUES_CFG)) { if (b && b.includes(k)) return k; }
  return b;
}
function initials(n) {
...
document.querySelectorAll('.seat').forEach(seat => {
  ...
  seat.addEventListener('click', () => {
    ...
    activeBloque === b ? clearHighlight() : highlightBloque(b);
  });
});
...
function highlightBloque(b) {
  activeBloque = b;
  document.querySelectorAll('.seat').forEach(s => s.classList.toggle('dimmed', s.dataset.bloque !== b));
  document.querySelectorAll('#leyenda .legend-item').forEach(item => {
...
// ── HOME: Bloques cards ────────────────────────────────────────────────────
const TOTAL = 60;
const bgEl = document.getElementById('bloquesGrid');
Object.entries(BLOQUES_CFG).forEach(([nombre, cfg]) => {
  ...
  card.addEventListener('click', () => {
    ...
    activeFilter = nombre;
    document.querySelectorAll('.fbtn').forEach(b => b.classList.toggle('on', b.dataset.bloque === nombre));
    renderGrid();
  });
...
function renderGrid() {
  ...
  visible.forEach((leg, i) => {
    ...
    const temas = leg.tematicas.slice(0, 2).map(t =>
      `<span class="tag">${t.split(':')[0].split('/')[0].trim().slice(0, 22)}</span>`
    ).join('');
...
async function openPanel(idx) {
  ...
  function sec(title, items, cls, lCls) {
    if (!items || !items.length) {
      if (cls === 'green') html += `<div class="sec"><div class="sec-label green">${title}</div><p class="empty">No identificados</p></div>`;
      return;
    }
    html += `<div class="sec"><div class="sec-label ${cls||''}">${title}</div>
      <ul class="items ${lCls||''}">${items.map(i => `<li>${i}</li>`).join('')}</ul></div>`;
  }
  ...
  if (typeof COMISIONES !== 'undefined') {
    const misComisiones = COMISIONES.filter(c => c.integrantes.some(i => i.nombre === leg.nombre));
    if (misComisiones.length) {
      html += `<div class="sec"><div class="sec-label" style="color:#7c3aed;border-color:#ede9fe">Comisiones que integra</div><ul class="items">`;
      misComisiones.forEach(c => {
        const rol = c.presidente === leg.nombre ? ' — <strong>Preside</strong>'
          : c.vice1 === leg.nombre ? ' — Vice 1°'
          : c.vice2 === leg.nombre ? ' — Vice 2°' : '';
  ...
  const myNotes = await cargarNotas('notas', 'legislador', leg.nombre);
  if (myNotes.length) {
    ...
    myNotes.forEach(n => {
      ...
      html += `<li data-nota-id="${n.id}">
        ...
        <button onclick="borrarNotaLeg('${n.id}',${idx})" style="float:right;background:none;border:none;color:#f87171;cursor:pointer;font-size:.72rem;padding:0 4px">✕</button>
      </li>`;
    });
    html += `</ul></div>`;
  }
...
document.getElementById('overlay').addEventListener('click', e => {
  closePanel();
  if (typeof cerrarComision === 'function') cerrarComision();
});
document.addEventListener('keydown', e => { if (e.key === 'Escape') { closePanel(); if (typeof cerrarComision === 'function') cerrarComision(); } });
...
document.getElementById('saveNote').addEventListener('click', async () => {
  ...
  const result = await guardarNota('notas', { legislador: leg.nombre, texto, autor: autor || null });
  btn.textContent = 'Guardar nota'; btn.disabled = false;
  if (result) {
    document.getElementById('noteText').value = '';
    document.getElementById('noteAuthor').value = '';
...
async function borrarNotaLeg(id, idx) {
  if (!confirm('¿Eliminar esta nota?')) return;
  const ok = await eliminarNota('notas', id);
  if (ok) await openPanel(idx);
}
